...
class facelm():
    ...
    def landmarks(self,frame,poi):
        landmarks = []
        frameRGB = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = self.face.process(frameRGB)
        if (results.multi_face_landmarks) != None:
            for lms in results.multi_face_landmarks:
                self.draw_face.draw_landmarks(poi,lms,self.mp.solutions.face_mesh.FACE_CONNECTIONS, self.drawSpecC,self.drawSpecL)
                landmark = []
                for lm in lms.landmark:
                    landmark.append((int(poi_width*lm.x),int(poi_height*lm.x)))
...
while True:
    ...
    poi = np.zeros([poi_height,poi_width,3], dtype= np.uint8)
    facelandm.landmarks(frame,poi)
