        ...
        if results.multi_hand_landmarks != None:
            ...
            for handlandmarks in results.multi_hand_landmarks:
                ...
                for landmark in handlandmarks.landmark:
                    Landmarks.append((int(window_width *landmark.x),int(window_height * landmark.y), landmark.z))
...
def find_errors(known_gestures,unknown_gestures,keypoints):
    ...
    for row in keypoints:
        for column in keypoints:
            known_error += (known_gestures[row][column])
            unknown_error += (unknown_gestures[row][column])
    return known_error, unknown_error
...
while True:
    ...
    if not training:
        if Handlm != []:
            ...
            cv2.putText(frame,str(int(error[0])),(100,150),cv2.FONT_HERSHEY_SIMPLEX,3,(255,0,0),3)
            cv2.putText(frame,str(int(error[1])),(100,300),cv2.FONT_HERSHEY_SIMPLEX,3,(255,0,0),3)
    for hand in Handlm:
        for ind in keypoints:
            cv2.circle(frame,(hand[ind][0],hand[ind][1]),15,(0,255,0),3)
